...
const format_to_markdown = (data) => {
    ...
    function getSetName(card) {
        if (!card || !card.attributes.card_set_names || !card.attributes.card_set_names.length) return '';
    ...
    function groupDeckCardsByType(decklist) {
        ...
        const cardsInDeck = Object.keys(slots).map(id => getCardById(id)).filter(Boolean);
        const groups = {};
    ...
    const deckSubtitle = identity ? `[${identity.attributes.title}](${getCardLink(identity)}) _(Core Set)_` : '';
    // Group cards by type
    ...
    for (const type of typeOrder) {
        ...
        const cardsSorted = groups[type].slice().sort((a, b) => a.attributes.title.localeCompare(b.attributes.title));
        // Get total count for this type
        const total = cardsSorted.reduce((sum, card) => sum + (decklist.attributes.card_slots[card.id] || 0), 0);
    ...
    const minCards = identity && identity.attributes.minimum_deck_size ? identity.attributes.minimum_deck_size : 0;
    ...
    const decklistLink = decklist.id ? `Decklist [published on NetrunnerDB](https://netrunnerdb.com/en/decklist/${decklist.id}/${encodeURIComponent(deckName.replace(/\s+/g, '-').toLowerCase())}).` : '';
    return [
        deckTitle,
        '',
        deckSubtitle,
