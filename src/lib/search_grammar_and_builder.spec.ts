...
import { queryGrammar, CardSearchQueryBuilder, PrintingSearchQueryBuilder } from "./search_grammar_and_builder";
...
describe("Search Grammar and Builder", () => {
  ...
  test("CardSearchQueryBuilder - multiple terms", () => {
    ...
    expect(builder.parse_error).toBe(null);
    expect(builder.where).toBe("lower(unified_cards.stripped_text) LIKE ? AND unified_cards.cost = ?");
  ...
  test("CardSearchQueryBuilder - is_banned without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_banned) WHERE value = ?)");
  ...
  test("CardSearchQueryBuilder - is_restricted without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_restricted) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true"]);
  });
  ...
  test("CardSearchQueryBuilder - has_global_penalty without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_global_penalty) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true"]);
  });
  ...
  test("CardSearchQueryBuilder - is_banned with restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_banned) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_cards.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });

  test("CardSearchQueryBuilder - is_restricted with restriction", () => {
    const builder = new CardSearchQueryBuilder("is_restricted:true restriction_id:ban_list_foo");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_restricted) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_cards.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });

  test("CardSearchQueryBuilder - has_global_penalty with restriction", () => {
    const builder = new CardSearchQueryBuilder("has_global_penalty:true restriction_id:ban_list_foo");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_global_penalty) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_cards.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });
  ...
  test("CardSearchQueryBuilder - eternal_points", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_points) WHERE value = ?)");
  ...
  test("CardSearchQueryBuilder - universal_faction_cost", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.restrictions_universal_faction_cost) WHERE value = ?)");
    expect(builder.where_values).toEqual(["3"]);
  });

  test("CardSearchQueryBuilder - card_pool", () => {
    const builder = new CardSearchQueryBuilder("card_pool:best_pool");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.card_pool_ids) WHERE value = ?)");
  ...
  test("CardSearchQueryBuilder - format_id", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.format_ids) WHERE value = ?)");
  ...
  test("PrintingSearchQueryBuilder - multiple terms", () => {
    ...
    expect(builder.parse_error).toBe(null);
    expect(builder.where).toBe("lower(unified_printings.stripped_text) LIKE ? AND unified_printings.cost = ?");
    expect(builder.where_values).toEqual(["%trash%", "3"]);
    expect(builder.left_joins).toEqual([]);
  });
  ...
  test("PrintingSearchQueryBuilder - is_banned without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_banned) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true"]);
  });
  ...
  test("PrintingSearchQueryBuilder - is_restricted without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_restricted) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true"]);
  });
  ...
  test("PrintingSearchQueryBuilder - has_global_penalty without restriction", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_global_penalty) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true"]);
  });

  test("PrintingSearchQueryBuilder - is_banned with restriction", () => {
    const builder = new PrintingSearchQueryBuilder("is_banned:true restriction_id:ban_list_foo");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_banned) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_printings.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });

  test("PrintingSearchQueryBuilder - is_restricted with restriction", () => {
    const builder = new PrintingSearchQueryBuilder("is_restricted:true restriction_id:ban_list_foo");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_restricted) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_printings.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });

  test("PrintingSearchQueryBuilder - has_global_penalty with restriction", () => {
    const builder = new PrintingSearchQueryBuilder("has_global_penalty:true restriction_id:ban_list_foo");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_global_penalty) WHERE value = ?) AND EXISTS (SELECT 1 FROM json_each(unified_printings.restriction_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["true", "ban_list_foo"]);
  });
  ...
  test("PrintingSearchQueryBuilder - eternal_points", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_points) WHERE value = ?)");
    expect(builder.where_values).toEqual(["eternal_restriction_id=3"]);
  });
  ...
  test("PrintingSearchQueryBuilder - universal_faction_cost", () => {
    ...
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.restrictions_universal_faction_cost) WHERE value = ?)");
    expect(builder.where_values).toEqual(["3"]);
  });

  test("PrintingSearchQueryBuilder - card_pool", () => {
    const builder = new PrintingSearchQueryBuilder("card_pool:best_pool");
    expect(builder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.card_pool_ids) WHERE value = ?)");
    expect(builder.where_values).toEqual(["best_pool"]);
  });
  ...
  test("PrintingSearchQueryBuilder - bad date value throws", () => {
    ...
    }).toThrow('Invalid value "jul-22-2022" for date field "release_date" - only YYYY-MM-DD or YYYYMMDD are supported.');
  });
  ...
  test("Card & Printing - printings_released_by", () => {
    ...
    expect(cardBuilder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_cards.printings_released_by) WHERE value = ?)");
    expect(cardBuilder.where_values).toEqual(["best_org"]);

    const printBuilder = new PrintingSearchQueryBuilder("printings_released_by:best_org");
    expect(printBuilder.where.trim()).toBe("EXISTS (SELECT 1 FROM json_each(unified_printings.printings_released_by) WHERE value = ?)");
    expect(printBuilder.where_values).toEqual(["best_org"]);
  });
});
