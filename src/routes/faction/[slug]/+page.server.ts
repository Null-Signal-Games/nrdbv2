...
export const load: PageServerLoad = async ({ fetch, params, cookies }) => {
    ...
    return {
        ...
        decklists: new Promise(async (resolve, reject) => {
            try {
                const identities = await fetch(`${NRDB_API_URL}/cards?filter[faction_id]=${params.slug}&filter[card_type_id]=${card_type_id}&page[size]=50`)
                    .then((response) => response.json())
                    .then((response) => response.data as Card[])
                ...
                await Promise.all(
                    identities.map(async (identity: Card) => {
                        const decklists = await fetch(`${NRDB_API_URL}/decklists?filter[identity_card_id]=${identity.id}&page[size]=5`)
                            .then((response) => response.json())
