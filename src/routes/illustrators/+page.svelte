<script lang="ts">
	...
	let { data }: { data: { illustrators: Illustrator[], cards: ApiResponse<Printing>[] } } = $props();
				...
				{#if false}
					<div class="cards">
						{#each data.cards[index].data as card (card.id)}
							...
							<CardImage card={card} href={false} />
						{/each}
					</div>
				{/if}
				<p>
					{illustrator.attributes.name} ({illustrator.attributes.num_printings} cards)
				</p>
			</a>
		{/each}
	</div>
